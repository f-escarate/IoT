...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        gl_profile_tab[PROFILE_A_APP_ID].char_handle = 0x6666;
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        // If is a new read, we get a new message
        if(param->read.offset == 0){
            gen_message();
        }
        if (param->read.offset < messageLength(protocol) - 1) {
            rsp.attr_value.len = messageLength(protocol) - param->read.offset;
            rsp.attr_value.offset = param->read.offset;
            memcpy(rsp.attr_value.value,
                    SEND_MESSAGE+param->read.offset, rsp.attr_value.len);
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            char ok = param->write.value[0];
            char change = param->write.value[1];
