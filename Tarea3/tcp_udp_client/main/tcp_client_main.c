...
extern char* tcp_client(char protocol, char transportLayer);
...
void app_main(void)
{
    ...
    char protocol = 5; char transport_layer = 20;

    char* response = NULL; char ok = 1;
    while(ok){
        // Selecting TCP = 0 or UDP = 1 in order to send the message
        if (20 <= transport_layer && transport_layer <= 2)
            response = tcp_client(protocol, transport_layer);
        else if (transport_layer == 23) 
